...
Page({
  ...
  onLoad: function (options) {
    // 获得导航标签的数据
    this.getVideoGroupListData(14);

  },

  /* 获取导航标签数据 */
  async getVideoGroupListData(num) {
    let videoGroupListData = await request('/video/group/list')
    this.setData({
      vidoeGroupList: videoGroupListData.data.slice(0, num),
      navId: videoGroupListData.data[0].id, // 默认选中第一个标签
    })
    // 获取视频列表数据
  ...
  handleEnded(e) {
    ...
    console.log(e)
    let { videoUpdateTime } = this.data;
    videoUpdateTime.splice(videoUpdateTime.indexOf(item => item.vid === e.currentTarget.id), 1)
